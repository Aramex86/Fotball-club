...
class AddEditMatch extends Component {
    ...
    updateFields = (match,teamsOptions,teams,type,matchId)=>{
        const newFormdata ={
            ...this.state.formdata
        }
    ...
    submitForm = (event) => {
        ...
        if (formIsValid) {
          if(this.state.formType === 'Edit Match'){
              dataBase.ref(`matches/${this.state.matchId}`).update(dataToSubmit).then(()=>{
                this.formSucces('Updated correctly')
              }).catch((e)=>{
                this.setState({
                    formError: true,
                  });
              ...
              dataBaseMatches.push(dataToSubmit).then(()=>{
	...
	componentDidMount() {
		...
		const getTeams = (match, type) => {
			dataTeams.once('value').then((snapshot) => {
                ...
                this.updateFields(match,teamsOptions,teams,type,matchId)
            });
        };
